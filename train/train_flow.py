...
def main():
    ...
    for step in range(start_step, args.num_steps):
        ...
        # DataParallel can't scatter text lists — pre-encode on primary device
        if args.data_parallel and 'text' in y and 'text_emb' not in y:
            with torch.no_grad():
                text_emb = raw_model.encode_text(y['text'], device=device)
            y['text_emb'] = text_emb
            del y['text']
