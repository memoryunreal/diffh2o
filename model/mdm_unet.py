...
class MDM_UNET(nn.Module):
    ...
    def __init__(self,
        ...
        self.motion_enc = motion_enc_frames > 0
        self.motion_enc_dim = motion_enc_dim * motion_enc_frames
        ...
        if self.cond_mode != 'no_cond':
            if 'text' in self.cond_mode:
                ...
                print('EMBED TEXT')
                print('Loading CLIP...', clip_version)
            ...
            if self.obj_enc:
                self.embed_object = nn.Linear(self.obj_enc_dim, self.latent_dim)
            if self.motion_enc:
                if self.motion_enc_frames > 1:
                    self.embed_motion = nn.Linear(self.motion_enc_dim, self.latent_dim)
                else:
                    self.embed_motion = nn.Linear(self.motion_enc_dim, self.latent_dim)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        #emb += self.embed_hand_label(self.mask_cond(torch.tensor(y['hand_label'],dtype=torch.float32).view(bs,-1).to(device), force_mask=force_mask))
        if 'text' in self.cond_mode:
        ...
        if self.obj_enc:
            emb += self.embed_object(self.mask_cond(torch.cat(y['object_bps']).view(bs,-1).to(device), force_mask=force_mask))
        if self.motion_enc:
            emb += self.embed_motion(self.mask_cond(y['motion_enc_gt'].view(bs,-1).to(device), force_mask=force_mask))
